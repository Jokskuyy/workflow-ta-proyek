...
2. 2.2 Usulan Solusi
   a. 2.2.1 Identifikasi Kebutuhan Fungsional (Dashboard Publik, Panel Admin, API, penghubung integrasi, deployment, dan pengoperasian layanan).
   b. 2.2.2 Identifikasi Kebutuhan Teknis — React SPA, Vercel Serverless API berbasis Node.js, Supabase, Unity WebGL, serta analitik utama yang terverifikasi. Express.js dan Umami hanya disebut aktif apabila ada bukti; pada laporan Iman keduanya merupakan jalur opsional, bukan API atau analitik antarmuka utama.
   c. 2.2.3 Identifikasi Kebutuhan Non-Fungsional — pemuatan adaptif, umpan balik pemuatan, cache, evaluasi Lighthouse, rancangan yang mengutamakan smartphone, keamanan (JWT, RLS, dan pembatasan jumlah permintaan), privasi, kemudahan penggunaan, aksesibilitas, dan keterpeliharaan. Jangan memakai target waktu muat tetap tanpa bukti pengukuran.
3. 2.3 Rancangan Proyek
   ...
   d. 2.3.4 Perancangan Integrasi Keamanan dan Analitik. Jelaskan batas antara rancangan skema, RLS, dan trigger dengan integrasi yang menggunakannya; nyatakan analitik utama serta jalur opsional berdasarkan bukti.
   e. 2.3.5 Perancangan data sesuai fokus peran. Laporan perancang database dapat memuat ERD lengkap; laporan integrator memakai Pemetaan Data Integrasi dan mengatribusikan ERD, relasi, RLS, serta trigger kepada pemiliknya.
   f. 2.3.6 Perancangan Antarmuka (rancangan Dashboard Publik dan Panel Admin).
...
2. 3.2 Metode Implementasi
   a. 3.2.1 Implementasi Back-end — Vercel Serverless Functions (Node.js), Supabase client, spesifikasi endpoint, serta respons/error. Skema, SQL, RLS, dan trigger hanya dibahas oleh peran pemiliknya.
   b. 3.2.2 Implementasi Frontend — React SPA, Dashboard Publik, Panel Admin, autentikasi, pembatasan akses halaman, operasi CRUD, pencarian, bantuan, tutorial, serta pemuatan WebGL adaptif.
...
3. 3.3 Konfigurasi dan Ketentuan Operasional Sistem
   a. 3.3.1 Environment variables dan konsistensi identifier `unity_object_name`.
...
4. 3.4 Laporan Implementasi Proyek
   ...
   b. 3.4.2 Hasil dan bukti implementasi sesuai spesifikasi API dan mekanisme integrasi.
...
5. 3.5 Hasil Pengujian Proyek
   a. 3.5.1 Pengujian otomatis web dan API sesuai bukti yang tersedia.
   b. 3.5.2 Black Box Testing.
   c. 3.5.3 Pengujian Lighthouse sebagai data laboratorium, bukan data pengguna nyata.
