...
def process_xml_file(filepath):
    # Determine the namespace mapping
    ...
    # 1. Process w:rFonts elements
    # We find all elements with tag matching rFonts in any namespace or local name
    for elem in root.iter():
        # Tag check (handles namespace or local name)
        tag_local = elem.tag.split('}')[-1]
        
        if tag_local == 'rFonts':
            # Check attributes and replace with Times New Roman, except if it's Symbol/Wingdings/Courier New
            for attr in ['ascii', 'hAnsi', 'eastAsia', 'cs']:
                ...
                if val and val not in ['Symbol', 'Wingdings', 'Courier New', 'Times New Roman']:
                    ...
                    modified = True
                # If the attribute doesn't exist but we want to make sure it defaults to Times New Roman,
                # we can explicitly set it if there's a theme attribute.
                
            # If theme attributes exist, remove them or make sure explicit attributes are set
        ...
        # 3. Process generic font typeface attributes in theme or other drawing elements
...
def walk_and_fix(directory):
    print(f"Recursively fixing fonts in: {directory}")
