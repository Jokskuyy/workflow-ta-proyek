...
def _current_docx_is_stale():
    """True when Markdown/manifest changed after the existing DOCX artifact."""
    if not CURRENT_DOCX.exists():
        return True
    docx_mtime = CURRENT_DOCX.stat().st_mtime
    return any(
        source.exists() and source.stat().st_mtime > docx_mtime
        for source in (DRAFT, IMAGE_MANIFEST)
    )
...
@pytest.mark.skipif(
    _current_docx_is_stale(),
    reason="formatted DOCX is older than Markdown/manifest; draft-first review pending",
)
