...
DOCX_STALE = (
    CURRENT_DOCX.exists()
    and CURRENT_DOCX.stat().st_mtime < max(DRAFT.stat().st_mtime, MANIFEST.stat().st_mtime)
)
# ``skills/scripts`` is the tracked source of truth.  The scratch copy is
# generated by build_pipeline.py and may not exist yet in a clean clone.
VALIDATOR = SKILLS / "validate_docx_structure.py"
REF_CAPTIONS = FIXTURES / "reference_caption_numbers.json"
REF_VALIDATOR = FIXTURES / "reference_validator_summary.json"
REFERENCE_BASELINE_STALE = (
    REF_CAPTIONS.exists()
    and REF_CAPTIONS.stat().st_mtime < max(DRAFT.stat().st_mtime, MANIFEST.stat().st_mtime)
)
...
@pytest.mark.skipif(
    DOCX_STALE or REFERENCE_BASELINE_STALE,
    reason="formatted DOCX or captured caption baseline is stale after an intentional report revision",
)
...
@pytest.mark.skipif(DOCX_STALE, reason="formatted DOCX is stale; draft-first review intentionally has not rebuilt it")
...
def test_scope_guard_generation_scripts_exist():
    # Tracked generation sources must exist even before runtime bootstrap.
...
def test_scope_guard_validator_and_injector_still_present():
    # The tracked non-goal sources must exist in a clean clone. Their scratch
    # copies are materialized by build_pipeline.py when the runtime starts.
